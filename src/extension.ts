...
export function activate(context: vscode.ExtensionContext) {
  ...
  const completionProvider = vscode.languages.registerCompletionItemProvider(
    { pattern: "**" }, //  For all languages - adjust language filter as needed (e.g., { language: 'typescript' })
    new CodeCompletionProvider(),
    "." // Trigger characters -  e.g., ['.', '(', '"', `''] -  adjust based on when you want completions to trigger
  );
